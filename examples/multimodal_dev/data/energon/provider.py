...
def _require_keyword_only_parameter(function: Callable[..., Any], owner: str, name: str) -> None:
    ...
    if parameter is None or parameter.kind is not Parameter.KEYWORD_ONLY or parameter.default is not Parameter.empty:
...
def _build_task_encoder(api: Energon7Api, args: Any) -> Any:
    ...
    if factory_spec is None:
        raise NotImplementedError(f"Model {model_arch!r} does not define energon_task_encoder_factory")
    ...
    if not isinstance(task_encoder, api.task_encoder_type):
        raise TypeError(
            f"Model {model_arch!r} energon_task_encoder_factory must return " "a megatron.energon.TaskEncoder"
        )
    return task_encoder
...
def _validation_datasets(entries: Any) -> list[Any]:
    ...
    for entry in entries:
        if not isinstance(entry, (tuple, list)) or len(entry) != 2:
            raise TypeError("Each Energon validation dataset entry must be a (dataset, factory) pair")
...
def train_valid_test_datasets_provider(_train_val_test_num_samples: Any):
    ...
    train_iterator = iter(
        _build_loader(api, train_dataset, savable=True, prefetch_factor=args.energon_prefetch_factor)
    )

    valid_iterators = None
    if int(getattr(args, "eval_iters", 0)) > 0:
        entries = api.get_val_datasets(args.energon_path, split_part=args.energon_val_split, **common)
        valid_iterators = [
            iter(_build_loader(api, dataset, savable=False, prefetch_factor=args.energon_prefetch_factor))
