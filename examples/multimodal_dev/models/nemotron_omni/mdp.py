...
"""Model-owned MDP adapter and replay for canonical Nemotron Omni."""
...
class NemotronOmniMdpAdapter(Qwen35VLMdpAdapter):
    ...
    def encode(self, encoder, payload, layout):
        ...
        encoded = encode_nemotron_omni_images(encoder, payload, grids)
        return MdpEncoderOutput(planes=(encoded,))
...
def nemotron_omni_mdp_replay(model, batch, record, encoder_leaves: tuple):
    """Replay zero or one canonical projected plane through the native model."""
    if len(encoder_leaves) > 1:
        raise RuntimeError(
            "Nemotron Omni MDP replay accepts at most one encoder output plane; "
