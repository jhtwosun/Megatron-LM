...
def test_claim_failure_does_not_abort_unclaimed_or_wrong_owner(monkeypatch):
    api = _api()
    _runtime_value, _outputs, owner, _native, prepared = _parts(contributor=False)
    gate = object.__new__(_D3EncoderCompletionGateBinding)
    monkeypatch.setattr(api, "_capture_prepared_authority", _test_outer_authority)
    prepared.producer.owner = object()

    def fail_claim(_binding, _prepared):
        raise MdpTaskFatalError("injected Gate-4 claim failure")

    monkeypatch.setattr(_D3EncoderCompletionGateBinding, "claim_for_backward", fail_claim)
    try:
        with pytest.raises(MdpTaskFatalError, match="claim failure"):
...
@pytest.mark.parametrize("fault", ("foreign", "native-substitution", "owner-substitution"))
def test_claim_substitution_is_task_fatal_before_backward(monkeypatch, fault):
    api = _api()
    _runtime_value, _outputs, owner, native, prepared = _parts(contributor=False)
    if fault == "foreign":
        result = object()
    elif fault == "native-substitution":
        object.__setattr__(prepared, "native_completion", object())
        result = native
    else:
        prepared.producer.owner = object()
        result = native
    gate = _gate(monkeypatch, prepared, result=result)
    monkeypatch.setattr(
        EncoderForwardHandle,
        "backward",
...
@pytest.mark.skipif(not torch.cuda.is_available(), reason="requires CUDA encoder graph")
def test_backward_reentrancy_poisons_gate_and_makes_outer_fatal(monkeypatch):
    api = _api()
    _runtime_value, _outputs, owner, _native, prepared = _parts()
    gate = _gate(monkeypatch, prepared)
    original_backward = EncoderForwardHandle.backward
...
@pytest.mark.skipif(not torch.cuda.is_available(), reason="requires CUDA real Gate-4 workspace")
def test_real_reentrant_gate4_claim_poisons_binding_and_aborts_outer(monkeypatch):
    api = _api()
    binding, context, prepared, bound, owner, original = _real_gate4_parts(monkeypatch)
    mint_finalize_ready = api._mint_finalize_ready

    def reenter(*args):
        ready = mint_finalize_ready(*args)
        with pytest.raises(MdpTaskFatalError, match="cannot be replayed"):
            api._execute_d3_encoder_backward(binding, prepared)
        return ready
