...
def test_actual_25_layer_vision_and_four_layer_decoder_image_plus_text_backward(vision_recompute):
    ...
    try:
        ...
        packed_leaf = torch.cat(tuple(plane.detach() for plane in captured_planes), dim=-1)
        ...
        replay_loss = model(
            ...
            pixel_values=None,
            image_grid_thw=image[4],
            vision_embeddings=packed_leaf,
        )
        torch.testing.assert_close(replay_loss, image_loss)
        loss = (
            (image_loss * image[2]).sum()
            + (replay_loss * image[2]).sum()
            + (text_loss * text[2]).sum()
        )
        ...
        assert packed_leaf.grad is not None
        assert torch.isfinite(packed_leaf.grad).all()
        assert float(packed_leaf.grad.abs().sum()) > 0
