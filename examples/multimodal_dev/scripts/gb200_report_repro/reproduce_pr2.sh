...
CELL_ROWS=(
    "pr2_baseline|${PR2_SHA}|pr2|1|ordinary|none|0|10|median|20619.3|45.9244|101708.21|reserved=101.0977_GiB|PR2 prior-chain ordinary-loader report"
)

usage() {
    cat <<'EOF'
Reproduce the standalone PR2 GB200 performance report.

Usage:
  reproduce_pr2.sh [options]
...
Options:
  ...
  --cells CSV                 Run only the listed PR2 cells.
  --dry-run                   Prepare checkouts and print launcher commands.
  --prepare-only              Prepare checkouts and metadata, then stop.
  -h, --help                  Show this help.

This file contains its host runner, container-side runner, PR2 cell matrix,
...
run_node() {
    ...
    if [[ "${STACK_LEVEL}" != "pr2" ]]; then
        echo "PR2 runner requires STACK_LEVEL=pr2" >&2
        exit 2
    fi
...
lines = [
    "# GB200 PR2 reproduction summary", "",
...
printf '%s\n' "${PR2_SHA}" > "${result_root}/stack_code_revisions.txt"

if [[ "${prepare_only}" == "1" ]]; then
    echo "Prepared ${#selected_rows[@]} PR2 cells under ${checkout_root}"
