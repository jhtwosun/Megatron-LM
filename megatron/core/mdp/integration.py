...
def _resolve_vision_capture_mode(args, config: MdpConfig) -> VisionCaptureMode:
    ...
    static_provider = getattr(args, "dataset_provider", None) == "mdp_mock" or (
    ...
    if not config.dynamic_encoder_cp and static_provider:
        if (
            ...
            raise MdpConfigurationError(
                "MDP: static locator capture requires TP1, packed THD, and no window overlap."
            )
        return mode
...
def mdp_config_from_args(args) -> MdpConfig:
    ...
    return MdpConfig(
        ...
        encoder_assignment_policy=getattr(args, "mdp_encoder_assignment_policy", "lpt"),
    ...
    try:
        runtime = MdpRuntime(
            ...
            planner=MdpPlanner(
                ...
                assignment_policy=mdp_config.encoder_assignment_policy,
            ),
