...
def derive_image_grid_thw(*, width: int, height: int) -> tuple[int, int, int]:
    ...
    resized_height, resized_width = smart_resize(
        height,
        width,
        factor=_PATCH_SIZE * _SPATIAL_MERGE_SIZE,
...
def validate_image_metadata(descriptors: Sequence[Mapping[str, Any]], image_grid_thw: Any):
    ...
    for item_index, (descriptor, raw_grid) in enumerate(zip(descriptors, grids)):
        ...
        if (
            ...
            and _grid_tuple(descriptor_grid, f"image descriptor {item_index} grid_thw")
            != expected_grid
        ):
            raise ValueError(
...
class Qwen35EnergonTaskEncoder(TaskEncoder):
    ...
    def __init__(
        ...
        *,
        ...
        if self.seq_length % self.alignment:
            raise ValueError(
                f"seq_length={self.seq_length} must be divisible by alignment={self.alignment}"
            )
        ...
        if seen_images < num_images:
            ...
            conversation[first_user]["content"] = [
                ...
                *(
                    ...
                    for index in range(seen_images, num_images)
                ),
            ]
        return conversation
    ...
    def _validate_image_carrier(cls, value: Any, location: str, *, sequence: bool = False) -> None:
        ...
        is_pil_image = any(
            base.__module__ == "PIL.Image" and base.__name__ == "Image"
        ...
        if grid is None:
            try:
                grid = derive_image_grid_thw(
                    width=descriptor.get("width"), height=descriptor.get("height")
                )
            except ValueError as exc:
                raise ValueError(
                    f"image descriptor {index} requires authoritative grid_thw or "
        ...
        for index, value in enumerate(values):
            ...
            if "encoded_image" in descriptor:
                self._validate_image_carrier(
                    descriptor["encoded_image"], f"image_descriptors[{index}].encoded_image"
                )
            if "encoded_images" in descriptor:
                self._validate_image_carrier(
                    descriptor["encoded_images"],
                    f"image_descriptors[{index}].encoded_images",
                    sequence=True,
                )
    ...
    def preencode_sample(self, sample: Mapping[str, Any]) -> dict[str, Any]:
        ...
        if sample_type.__name__ == "VQASample" and sample_type.__module__.startswith(
            "megatron.energon"
        ):
        ...
        for sample in samples:
            ...
            if current and (
                ...
                or (
                    ...
                    and len(current) >= self.max_samples_per_sequence
                )
            ):
    ...
    def _validate_packed_group(self, samples: list[dict[str, Any]]) -> None:
        ...
        if not self.use_packed_sequence and len(samples) != 1:
            raise ValueError(
                "multi-document Energon groups require packed THD; BSHD groups must be singleton"
            )
        if (
            sum(self._padded_length(int(sample["input_ids"].numel())) for sample in samples)
            > self.seq_length
        ):
...
def _parallel_alignment(args: Any) -> int:
    tp = _positive_integer(
        getattr(args, "tensor_model_parallel_size", 1), "tensor_model_parallel_size"
    )
