...
def _project_leaf_gradients(
    *,
    ...
    if not set(item_gradients).issubset(manifest_item_ids):
        raise MdpPlanError("MDP: encoder-only gradient views belong to the exact manifest.")
    # Replay follows decoder assignments; canonicalize ID bindings, never leaf rows.
    item_gradients = {
        item_id: item_gradients[item_id]
        for item_id in manifest_item_ids
        if item_id in item_gradients
    }
