...
_MATERIALIZER_VALIDATOR = f"{_MODULE}.validate_image_metadata"
_PROVIDER = "examples.multimodal_dev.data.energon.provider.train_valid_test_datasets_provider"
...
def _sample(*, grids=(), carriers=(), text="final answer"):
    ...
    for index, (grid, carrier) in enumerate(zip(grids, carriers)):
        descriptors.append(
            {"kind": "image_path", "path": carrier, "grid_thw": grid, "identity": index}
        )
...
def test_registry_keeps_qwen_energon_hooks_lazy_and_model_owned():
    ...
    assert entry["energon_image_metadata_validator"] == _MATERIALIZER_VALIDATOR
    assert entry["dataset_providers"]["energon"] == _PROVIDER


def test_materializer_validates_all_grids_before_processor_or_decode(monkeypatch):
    qwen = _module()
    generic = importlib.import_module("examples.multimodal_dev.data.energon.materializer")
    calls = []
    descriptors = (
        {"kind": "image_path", "path": "/first.jpg", "grid_thw": (1, 2, 2)},
        {"kind": "image_path", "path": "/second.jpg", "grid_thw": (1, 4, 2)},
    )
    monkeypatch.setattr(qwen, "_image_processor", lambda: calls.append("processor"))
    monkeypatch.setattr(generic, "load_descriptor_image", lambda *_args: calls.append("decode"))

    with pytest.raises(ValueError, match="descriptor 1 grid_thw does not match"):
        qwen._materialize_images(descriptors, torch.tensor([[1, 2, 2], [1, 2, 2]]))
    assert calls == []


def test_owner_materialization_decodes_once_and_stays_on_cpu(monkeypatch):
    qwen = _module()
    generic = importlib.import_module("examples.multimodal_dev.data.energon.materializer")
    output = io.BytesIO()
    Image.new("RGB", (256, 256), (41, 97, 173)).save(output, format="JPEG")
    ...
    pixels = qwen.build_image_materializer(args=SimpleNamespace())(
        (descriptor,), torch.tensor([grid], dtype=torch.long)
    )

    assert calls == [descriptor]
    assert pixels.device.type == "cpu"
    assert pixels.dtype == torch.float32
    assert pixels.shape == (grid[0] * grid[1] * grid[2], 1536)
...
def test_metadata_preencode_never_opens_or_decodes_image_and_keeps_carrier_identity(monkeypatch):
    ...
    assert image_positions.tolist() == list(
        range(int(image_positions[0]), int(image_positions[0]) + 4)
    )


def test_shifted_labels_and_loss_mask_supervise_assistant_text_only():
    document = _encoder().preencode_sample(
        _sample(grids=((1, 2, 2),), carriers=(b"opaque",), text="final answer")
    )
...
def test_multiple_images_preserve_source_order_grids_and_distinct_positions():
    ...
    document = _encoder().preencode_sample(
        _sample(grids=((1, 2, 2), (1, 4, 2)), carriers=(first, second))
    )
...
def test_selection_and_batch_preserve_document_boundaries_and_source_order():
    ...
    assert [envelope["documents"] for envelope in envelopes] == [
        tuple(documents[:2]),
        tuple(documents[2:]),
    ]
...
def test_native_thd_packer_preserves_qwen_documents_and_image_sidecar(monkeypatch):
    ...
    documents = [
        encoder.preencode_sample(
            _sample(grids=((1, 2, 2),), carriers=(b"first",), text="first answer")
        ),
        encoder.preencode_sample(
            _sample(grids=((1, 2, 2),), carriers=(b"second",), text="second answer")
        ),
    ]
    ...
    monkeypatch.setattr(window, "pixel_capture_owner_state", lambda: False)
...
@pytest.mark.parametrize(
    ("tp", "cp", "sp", "expected"),
    [(1, 1, False, 1), (2, 1, True, 2), (1, 2, False, 4), (2, 2, True, 8)],
)
def test_factory_alignment_matches_native_tp_cp_sequence_layout(tp, cp, sp, expected):
    qwen = _module()
    args = SimpleNamespace(
        tensor_model_parallel_size=tp, context_parallel_size=cp, sequence_parallel=sp
    )
        ...
        (
            {
                ...
                "json": {
                    "text": "user: <image> q assistant: a",
                    "image_descriptors": [{"path": "x.jpg"}],
                },
            },
            "grid_thw",
        ),
