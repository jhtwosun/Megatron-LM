...
class _Qwen3VLMdpMockDataset(MdpThdMockDataset):
    """Remove the model's video sentinel from shared random text tokens."""

    def __getitem__(self, index):
        sample = super().__getitem__(index)
        for name in ("input_ids", "labels"):
            sample[name][sample[name] == VIDEO_TOKEN_ID] = 1
        return sample
...
def mdp_mock_dataset_provider(train_val_test_num_samples):
    ...
    return tuple(
        _Qwen3VLMdpMockDataset(num_samples=num_samples, seed=1234 + split, **kwargs)
