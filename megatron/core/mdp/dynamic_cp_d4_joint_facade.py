...
"""Private joint decoder/encoder Dynamic-CP repeated-D4 composition."""
...
from megatron.core.mdp import dynamic_cp_d4_dynamic_decoder_replay as _replay
...
def _scheduled_abort(owner: _replay._D4DynamicDecoderReplayOwner, error: BaseException) -> None:
    owner.abort(error)


def _run_repeated_d4_joint_iteration(
    runtime: Any,
    capture_owner: Any,
    *,
    ...
    """Run one joint decoder/encoder Dynamic-CP iteration through Gate7."""
    if type(native_schedule_args) is not tuple or (
        ...
        raise MdpConfigurationError("MDP: joint facade native arguments are immutable and mapped.")
    ...
    try:
        projection = _gather_d4_source_catalog(
            capture_owner, binding, expected_dynamic_decoder_cp=True
        )
        ...
        authority = build_repeated_d4_joint_iteration_authority(
            ...
            decoder_minimum_cp_size=decoder_minimum_cp_size,
    ...
    replay = _advance(
        ...
        lambda: _replay.run_repeated_d4_dynamic_decoder_replay(
            publication,
            authority,
    ...
    native = _advance(
        ...
        lambda: _native._run_d4_dynamic_native_schedule(
    ...
    gradient = _advance(
        ...
        lambda: _gradient._run_repeated_d4_dynamic_encoder_gradient_from_replay(
            replay,
            authority,
            native.completion,
            all_to_all_single=all_to_all_single,
