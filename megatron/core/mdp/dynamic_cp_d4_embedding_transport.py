...
def _prepare_repeated_d4_embedding(
    ...
    *,
    ...
    """Prepare one embedding exchange without starting Gate1 or a collective."""
    ...
    if not exact_coverage:
        raise MdpPlanError("MDP: repeated-D4 embedding outputs exactly cover local item authority.")
...
def _execute_repeated_d4_embedding(
    binding: _RepeatedD4GroupBinding,
    prepared: PreparedDynamicBridgeExchange,
    *,
    all_to_all_single: Callable[..., Any] = dist.all_to_all_single,
) -> PreparedDynamicBridgeExchange:
    """Execute only the already-prepared physical domain embedding A2A."""
    _execute_validated_dynamic_bridge_exchange(
...
def run_repeated_d4_embedding(
    ...
    *,
    ...
    def prepare():
        return _prepare_repeated_d4_embedding(
            binding,
            authority,
    ...
    def execute(prepared: PreparedDynamicBridgeExchange) -> PreparedDynamicBridgeExchange:
        return _execute_repeated_d4_embedding(
