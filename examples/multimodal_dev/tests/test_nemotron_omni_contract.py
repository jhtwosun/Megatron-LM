...
def test_mdp_singleton_replay_matches_native_value_input_and_parameter_gradients():
    mdp_module = _load("mdp")
    ...
    batch = MappingProxyType({"input_ids": input_ids, "image_grid_thw": torch.tensor([[1, 2, 4]])})
    record = SimpleNamespace(decoder_packed_seq_params=object())
    replay_output = mdp_module.nemotron_omni_mdp_replay(mdp_model, batch, record, (mdp_leaf,))
    ...
    calls = []
    with pytest.raises(RuntimeError, match="one|single"):
        mdp_module.nemotron_omni_mdp_replay(
            lambda **kwargs: calls.append(kwargs), batch, record, (mdp_leaf, mdp_leaf)
        )
    assert calls == []
...
def test_invalid_configuration_fails_before_adapter_groups_encoder_or_ledgers(
    ...
    registry_entry = dict(MODEL_REGISTRY["nemotron_omni"])
    real_adapter_factory = registry_entry["mdp_adapter_factory_fn"]

    def counted_adapter(*args, **kwargs):
        calls["adapter"] += 1
        return real_adapter_factory(*args, **kwargs)

    registry_entry["mdp_adapter_factory_fn"] = counted_adapter
    registry_entry["vision_config_fn"] = lambda **_kwargs: vision_config
    monkeypatch.setitem(MODEL_REGISTRY, "nemotron_omni", registry_entry)
    monkeypatch.setattr(
        pretrain_multimodal, "core_transformer_config_from_args", lambda _args: language_config
    )
    overrides = (("recompute_granularity", "selective"),) if failure == "vision-recompute" else ()
    mdp_config = SimpleNamespace(
        vision_config_overrides=overrides, encoder_cp=2 if failure == "encoder-cp" else 1
    )
    ...
    def fail_resource(name):
        def unexpected(*_args, **_kwargs):
            calls[name] += 1
            raise AssertionError(f"{name} construction was reached")

        return unexpected

    monkeypatch.setattr(mdp_integration, "build_rank_map", fail_resource("rank_map"))
    monkeypatch.setattr(mdp_integration, "install_mdp_process_groups", fail_resource("groups"))
    monkeypatch.setattr(mdp_integration, "build_encoder_domain", fail_resource("encoder"))
    monkeypatch.setattr(mdp_integration, "MdpPlanner", fail_resource("planner"))
    monkeypatch.setattr(mdp_integration, "MdpEmbeddingStorage", fail_resource("storage"))
    monkeypatch.setattr(mdp_integration.ModalityBridge, "__init__", fail_resource("bridge"))
    monkeypatch.setattr(mdp_integration.ModalityBridge, "build_ledger", fail_resource("ledger"))
    ...
    args = SimpleNamespace(
        ...
        vision_num_layers=1,
        model_variant=None,
        world_size=2 if failure in {"tp", "pp", "decoder-cp", "encoder-cp"} else 1,
        tensor_model_parallel_size=language_config.tensor_model_parallel_size,
        pipeline_model_parallel_size=language_config.pipeline_model_parallel_size,
        context_parallel_size=language_config.context_parallel_size,
