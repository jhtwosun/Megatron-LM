...
@pytest.mark.parametrize(
    ...
    ((1, 512, 1), (1, 256, 2), (1, 128, 4), (4, 512, 1), (4, 256, 2), (4, 128, 4)),
    ids=("ep1-ecp1", "ep1-ecp2", "ep1-ecp4", "ep4-ecp1", "ep4-ecp2", "ep4-ecp4"),
)
def test_public_qwen_reads_actual_locator_storage_once_per_selected_rank(
    monkeypatch,
    mdp_group_registry,
    ...
    """Exercise the registered Qwen v2 capture and Gate0 path on shared files."""
    ...
    args = launch_args(actual_data_fixture, "qwen35_vl")
    ...
    expected_locators = tuple(
        ...
        for descriptor in actual_data_fixture.descriptors("qwen35_vl", lane)
    )
    ...
    if expert_parallel_size == 1:
        ...
        class Decoder(torch.nn.Module):
            ...
            def forward(self, *, input_ids, vision_embeddings, packed_seq_params, **_kwargs):
                assert vision_embeddings is not None and vision_embeddings.requires_grad
                value = vision_embeddings.float().square().mean()
                local_tokens = input_ids.shape[-1] // packed_seq_params.cp_group.size()
    ...
    document = storage_document(actual_data_fixture, "qwen35_vl", dist.get_rank())
    wrapped = integration.maybe_wrap_forward_backward(native_schedule, config)
    ...
    if selected:
        assert locator_h2d[0][1:3] == (512, _PATCH_DIM)
    ...
    published = observed["publications"]
    expected_ids = tuple(item.item_id for item in authority.global_manifest.items)
    assert published == ([expected_ids] if dist.get_rank() in (0, 4) else [()])
            ...
            [
                ...
                for ordinal, locator in enumerate(
                    tuple(
                        ...
                        for descriptor in actual_data_fixture.descriptors("qwen35_vl", rank // 4)
                    )
                )
            ]
...
def test_public_qwen_actual_locator_matches_fresh_ecp4_reference(
    monkeypatch,
    mdp_group_registry,
    actual_data_fixture,
    dynamic_decoder,
    ...
    def run(capacity, ecp):
        ...
        return run_actual_locator_parity_arm(
            ...
            adapter_module=mdp_adapter_module,
    ...
    def validate_parity():
        torch.testing.assert_close(candidate[0], reference[0], rtol=0.01, atol=0.01)
        assert_actual_gradients_close(candidate[1], reference[1], rtol=0.01)
        assert_actual_gradients_close(candidate[2], reference[2], rtol=0.01)
        torch.testing.assert_close(candidate[3], reference[3], rtol=0.01, atol=0.01)

    assert_world_check(validate_parity)


@pytest.mark.parametrize("dynamic_decoder", (False, True), ids=("fixed-cp4", "joint-dcp"))
@pytest.mark.parametrize("fault", ("malformed", "missing", "corrupt"))
def test_public_qwen_actual_locator_failure_then_same_runtime_retry(
    monkeypatch, mdp_group_registry, actual_data_fixture, dynamic_decoder, fault
):
    """Converge storage failures before publication, then reuse the exact runtime."""
    from examples.multimodal_dev.data.energon import materializer as generic
    ...
    integration._RUNTIME = runtime
    monkeypatch.setattr(
        forward_step, "get_args", lambda: launch_args(actual_data_fixture, "qwen35_vl")
    )
    reads = []
    native_materialize = generic.vision_locator_image_bytes
    ...
    def observe_bind(*args, **kwargs):
        bind_calls.append(kwargs["membership"].group_size)
        return native_bind(*args, **kwargs)

    def observe_encoder(*args, **kwargs):
        encoder_calls.append("encode")
        return native_encoder_forward(*args, **kwargs)

    def observe_publication(owner, **kwargs):
        publication_calls.append("gate1")
        return native_publication(owner, **kwargs)

    monkeypatch.setattr(mdp_adapter_module, "_bind_dynamic_encoder_cp", observe_bind)
    monkeypatch.setattr(encoder_ddp, "forward", observe_encoder)
    monkeypatch.setattr(facade._forward, "run_repeated_d4_encoder_publication", observe_publication)
    schedule_calls = []
    config = SimpleNamespace(
        dynamic_context_parallel=dynamic_decoder,
        min_dynamic_context_parallel_size=1,
    ...
    wrapped = integration.maybe_wrap_forward_backward(native_schedule, config)
    bad_document = storage_document(actual_data_fixture, "qwen35_vl", dist.get_rank())
    if dist.get_rank() == 0:
        descriptors = [dict(value) for value in bad_document["image_descriptors"]]
        if fault == "malformed":
            descriptors[0]["path"] = "../escape.jpg"
        elif fault == "missing":
            descriptors[0]["path"] = f"{actual_data_fixture.root}/qwen35_vl/missing.jpg"
        else:
            descriptors[0]["path"] = f"{actual_data_fixture.root}/qwen35_vl/table/images.parquet"
        bad_document["image_descriptors"] = tuple(descriptors)

    with pytest.raises(MdpPlanError) as raised:
        wrapped(data_iterator=iter(([bad_document],)), num_microbatches=1, forward_only=False)
    errors = [None] * dist.get_world_size()
    dist.all_gather_object(errors, (type(raised.value).__name__, str(raised.value)))
    assert len(set(errors)) == 1
    assert schedule_calls == []
    assert bind_calls == encoder_calls == publication_calls == []
    assert runtime.iteration == 0 and runtime.state is MdpRuntimeState.EMPTY
    assert runtime.storage.get_leaf(0) is None and allocator._outstanding == 0
    assert all(
    ...
    gathered_reads = [None] * dist.get_world_size()
    dist.all_gather_object(gathered_reads, tuple(reads))
    if fault == "malformed":
        assert gathered_reads == [()] * 8
    else:
        fault_path = (
            f"{actual_data_fixture.root}/qwen35_vl/missing.jpg"
            if fault == "missing"
            else f"{actual_data_fixture.root}/qwen35_vl/table/images.parquet"
        )
        assert gathered_reads[0][0] == gathered_reads[1][0] == fault_path

    good_document = storage_document(actual_data_fixture, "qwen35_vl", dist.get_rank())
    loss = wrapped(data_iterator=iter(([good_document],)), num_microbatches=1, forward_only=False)
    assert torch.isfinite(loss)
...
@pytest.mark.parametrize("dynamic_decoder", (False, True), ids=("fixed-cp4", "joint-dcp"))
def test_public_qwen_locator_text_only_has_zero_io_or_h2d(
    monkeypatch, mdp_group_registry, actual_data_fixture, dynamic_decoder
):
    class TextDecoder(torch.nn.Module):
        ...
        def __init__(self):
            super().__init__()
            self.scale = torch.nn.Parameter(torch.tensor(1.25, device="cuda"))

        def forward(self, *, input_ids, packed_seq_params, vision_embeddings, **_kwargs):
            assert vision_embeddings is None
            local_tokens = input_ids.shape[-1] // packed_seq_params.cp_group.size()
            return self.scale.square().expand(input_ids.shape[0], local_tokens)

    runtime = _runtime(
        group_registry=mdp_group_registry,
        encoder_capacity=128,
        vision_capture_mode=VisionCaptureMode.STABLE_LOCATOR_CATALOG,
        allocator=RecordingAllocator(),
    )
    run_text_only_public(
