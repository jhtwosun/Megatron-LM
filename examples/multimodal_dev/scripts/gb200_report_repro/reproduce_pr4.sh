...
CELL_ROWS=(
    "pr2_baseline|${PR2_SHA}|pr2|1|ordinary|none|0|10|median|20619.3|45.9244|101708.21|reserved=101.0977_GiB|PR2 comparison boundary"
    "pr4_mdp_off|${PR4_SHA}|pr4|1|mdp_off|none|0|10|median|19745.3|NA|106210.19|reserved=101.0566_GiB|PR4 default path with MDP disabled"
    "pr4_mdp_on|${PR4_SHA}|pr4|1|mdp_on|none|0|10|median|18910.4|NA|110899.40|reserved=76.6074_GiB|PR4 CP-local MDP"
)

usage() {
    cat <<'EOF'
Reproduce the standalone PR4 GB200 performance report.

Usage:
  reproduce_pr4.sh [options]
...
Options:
  ...
  --cells CSV                 Run only the listed PR4 cells.
  --dry-run                   Prepare checkouts and print launcher commands.
  --prepare-only              Prepare checkouts and metadata, then stop.
  -h, --help                  Show this help.

This file contains its host runner, container-side runner, PR4 cell matrix,
...
run_node() {
    ...
    case "${STACK_LEVEL}" in
        pr2|pr4) ;;
        *) echo "PR4 runner requires STACK_LEVEL=pr2 or pr4" >&2; exit 2 ;;
    esac
    ...
    if [[ "${STACK_LEVEL}" == "pr4" ]]; then
        common_extra+=(--mdp-inner-dp-scope cp)
    fi
...
lines = [
    "# GB200 PR4 reproduction summary", "",
...
printf '%s\n' "${PR2_SHA}" "${PR4_SHA}" > "${result_root}/stack_code_revisions.txt"

if [[ "${prepare_only}" == "1" ]]; then
    echo "Prepared ${#selected_rows[@]} PR4 cells under ${checkout_root}"
