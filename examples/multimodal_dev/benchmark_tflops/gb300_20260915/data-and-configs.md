...
The four-way variants differ in MDP mode and fused packing deliberately. The ordinary/nonfused cap is0; the fused-window cap131072 must be interpreted using the actual window builder, not as a global image count. The reviewed fixed sweep reports cap evaluation before owner filtering and1/1/2/4 fused packs as image area increases. All four unprofiled measurements completed. In original job752159, only ordinary and nonfused MDP profiles completed; its outer job timed out during packing-profile startup and latest never started. Later jobs753568/753569 completed the fused packing/latest profiles, with finalized sixteen-worker traces. The [four-way profile summary](profile-summary.md) retains the LR/evaluation/rack differences rather than pretending these were one unchanged profiling allocation.
