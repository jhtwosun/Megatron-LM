...
CELL_ROWS=(
    "pr5_mdp_off|${PR5_SHA}|pr5|2|mdp_off|none|0|10|median|37782.3|NA|55506.20|reserved=111.7617_GiB|PR5 generic PP sidecar, MDP disabled"
    "pr5_mdp_on|${PR5_SHA}|pr5|2|mdp_on|none|0|10|median|29419.9|NA|71283.45|reserved=138.6836_GiB|PR5 replicated PPxCP vision"
)

usage() {
    cat <<'EOF'
Reproduce the standalone PR5 GB200 performance report.

Usage:
  reproduce_pr5.sh [options]
...
Options:
  ...
  --cells CSV                 Run only the listed PR5 cells.
  --dry-run                   Prepare checkouts and print launcher commands.
  --prepare-only              Prepare checkouts and metadata, then stop.
  -h, --help                  Show this help.

This file contains its host runner, container-side runner, PR5 cell matrix,
...
run_node() {
    ...
    if [[ "${STACK_LEVEL}" != "pr5" ]]; then
        echo "PR5 runner requires STACK_LEVEL=pr5" >&2
        exit 2
    fi
...
lines = [
    "# GB200 PR5 reproduction summary", "",
...
printf '%s\n' "${PR5_SHA}" > "${result_root}/stack_code_revisions.txt"

if [[ "${prepare_only}" == "1" ]]; then
    echo "Prepared ${#selected_rows[@]} PR5 cells under ${checkout_root}"
