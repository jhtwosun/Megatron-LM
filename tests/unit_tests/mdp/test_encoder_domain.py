...
def test_encoder_model_parallel_groups_are_the_same_rank_local_singleton():
    world = torch.distributed.get_world_size()
    rank_map = build_rank_map(MdpRankSpec(world_size=world, tp=1, pp=2, cp=1, ep=1, encoder_cp=1))
    groups = install_mdp_process_groups(rank_map, group_registry=MdpGroupRegistry())
    encoder_pgs = build_encoder_pg_collection(rank_map, encoder_cp=1, process_groups=groups)

    assert encoder_pgs.tp is encoder_pgs.pp
    assert encoder_pgs.tp is encoder_pgs.ep
    assert encoder_pgs.tp is encoder_pgs.cp
    assert encoder_pgs.tp is encoder_pgs.expt_dp
    assert torch.distributed.get_world_size(encoder_pgs.cp) == 1
