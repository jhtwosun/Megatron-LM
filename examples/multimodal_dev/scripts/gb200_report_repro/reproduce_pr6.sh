...
CELL_ROWS=(
    "pr6_mdp_off|${PR6_CODE_SHA}|pr6|2|mdp_off|recompute|0|6|mean|30481.0|31.26|68801.94|allocated=96974.57_MB|cf12 exact-tip world64 rerun reference"
    "pr6_mdp_on|${PR6_CODE_SHA}|pr6|2|mdp_on|recompute|0|6|mean|24086.0|39.71|87069.33|allocated=127735.80_MB|cf12 exact-tip world64 rerun reference"
    "pr6_fused_retain_131072|${PR6_CODE_SHA}|pr6|2|mdp_fused|retain|131072|6|mean|13030.0|74.25|160947.97|allocated=128383.62_MB|safe-cap exact-tip world64 rerun reference"
    "pr6_fused_recompute_131072|${PR6_CODE_SHA}|pr6|2|mdp_fused|recompute|131072|6|mean|12775.0|74.79|164160.63|allocated=92029.23_MB|safe-cap exact-tip world64 rerun reference"
)

usage() {
    cat <<'EOF'
Reproduce the standalone PR6 GB200 performance report.

Usage:
  reproduce_pr6.sh [options]
...
Options:
  ...
  --cells CSV                 Run only the listed PR6 cells.
  --dry-run                   Prepare checkouts and print launcher commands.
  --prepare-only              Prepare checkouts and metadata, then stop.
  -h, --help                  Show this help.

This file contains its host runner, container-side runner, PR6 cell matrix,
...
run_node() {
    ...
    if [[ "${STACK_LEVEL}" != "pr6" ]]; then
        echo "PR6 runner requires STACK_LEVEL=pr6" >&2
        exit 2
    fi
...
lines = [
    "# GB200 PR6 reproduction summary", "",
...
printf '%s\n' "${PR6_CODE_SHA}" > "${result_root}/stack_code_revisions.txt"

if [[ "${prepare_only}" == "1" ]]; then
    echo "Prepared ${#selected_rows[@]} PR6 cells under ${checkout_root}"
